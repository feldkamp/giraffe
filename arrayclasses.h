...
namespace ns_arraydata{
	...
	public:
		...
		template <typename inputT> arraydata( const arraydata<inputT> *array ){
			copy( array );
		}
		...
		//-----------------------------------------------------------------------------------------copy functions
		template <typename inputT> void copy( const arraydata<inputT> *src ){
			p_data.resize(src->size());
			for (unsigned int i = 0; i < src->size(); i++){
				p_data[i] = (T) src->get_atIndex(i);
			}
		}
		...
		template <typename arrayT> void copy( const arrayT *src, const unsigned int arraysize ){
			p_data.resize( arraysize );
			if (arraysize > 0 && src){			
				//copy all elements from src to internal data object, casting them to type T
		...
		std::vector<T> vector() const		{ return p_data; }		//synonym for data(): returns internal STL vector
		
		//-----------------------------------------------------------------------------------------size/resize
		unsigned int size() const							{ return (int)p_data.size(); }	//total size of the array
		void resize( unsigned int newsize )					{ p_data.resize( newsize );  }	//resize internal array
	...
	////specialize for converting double arrays to boolean type
	////from c-arrays of doubles
	//template <> template <>
	//inline arraydata<bool>::arraydata( const double *doubleArray, const unsigned int size_val ){
	//	p_data.resize(size_val);
	//	for (unsigned int i = 0; i < size_val; i++){
	//		if ( doubleArray[i] > 0 ){
	//			set_atIndex(i, true);
	//		}else{
	//			set_atIndex(i, false);
	...
	public:
		...
		template <typename inputT> array1D( arraydata<inputT>* array )
				: arraydata<T>( array ) {
			setDim( array->size() );
		}
		template <typename inputT> array1D( arraydata<inputT>& array )
		...
		template <typename arrayT> array1D( const arrayT *CArray, unsigned int size_dim1 )
	...
	public:
		...
		template<typename inputT>array2D( array2D<inputT>* array ) 
				: arraydata<T>(array){
		...
		array2D( const pointerT *dataCArray, unsigned int size_dim1, unsigned int size_dim2 )
		...
		void setDim( unsigned int dim1, unsigned int dim2 ){
			if( dim1*dim2 != this->size() ){
				this->resize( dim1*dim2 );
			}
			p_dim1 = dim1;
			p_dim2 = dim2;
		}
	...
	public:
		...
		void setDim( unsigned int dim1, unsigned int dim2, unsigned int dim3 ){
			assert( dim1*dim2*dim3 == this->size() );
			p_dim1 = dim1;
			p_dim2 = dim2;
			p_dim3 = dim3;
		}	
	...
	public:
		...
		void setDim( unsigned int dim1, unsigned int dim2, unsigned int dim3, unsigned int dim4 ){
			assert( dim1*dim2*dim3*dim4 == this->size() );
			p_dim1 = dim1;
			p_dim2 = dim2;
			p_dim3 = dim3;
			p_dim4 = dim4;
	...
	//create different dimension arrays from this object
	template<typename T>
	void convertToArray1D( const arraydata<T> *src, array1D<T> *&dest ){
		delete dest;
		dest = new array1D<T>;
		dest->setDim( src->size() );
		dest->arraydata<T>::copy( src );
	}

	template<typename T>	
	void convertToArray2D( const arraydata<T> *src, array2D<T> *&dest, unsigned int dim1, unsigned int dim2){
		assert( dim1*dim2 == src->size() );
		delete dest;
		dest = new array2D<T>;
		dest->setDim( dim1, dim2 );
		dest->arraydata<T>::copy( src );
	}	
